...
const MOCK_ITEMS: Item[] = [
  {
    ...
    seller: '김철수',
    team: '마케팅팀',
    imageUrl: 'https://picsum.photos/seed/camping/600/600',
    donationPercent: 100
  },
  {
    id: '2',
  ...
  },
  {
    ...
    seller: '대표이사 J. Doe',
    team: '경영지원',
    imageUrl: 'https://picsum.photos/seed/golf/600/600',
    donationPercent: 100,
    isAuction: true,
    currentBid: 520000,
  ...
  {
    id: '4',
    title: '1:1 엑셀 마스터링 클래스',
    description: '고급 수식 및 매크로 1시간 과외',
    price: 30000,
    category: ItemCategory.TALENT,
    condition: ItemCondition.S,
    deliveryMethod: DeliveryMethod.DIRECT,
    seller: '박재무',
    team: '재무팀',
    imageUrl: 'https://picsum.photos/seed/excel/600/600',
    donationPercent: 100
  }
];
...
const App: React.FC = () => {
  ...
  const handleAddItem = (newItem: Partial<Item>) => {
    ...
    const itemToAdd: Item = {
      ...
      imageUrl: newItem.imageUrl || 'https://via.placeholder.com/600',
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-20 md:pb-0">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-30 px-4 py-4 md:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-gray-800">DS플리마켓</h1>
              <p className="text-[10px] text-teal-600 font-bold uppercase tracking-widest">우리 회사 따뜻한 장터</p>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-1 bg-gray-100 p-1 rounded-xl">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'market' ? 'bg-white text-teal-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              사내 장터
            </button>
            <button 
              onClick={() => setActiveTab('stats')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'stats' ? 'bg-white text-teal-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              기부 현황
            </button>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="bg-gray-900 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-lg hover:bg-gray-800 transition-all flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
              <span className="hidden sm:inline">물건 올리기</span>
            </button>
            <button 
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden animate-slideUp">
            <div className="relative">
              <img src={selectedItem.imageUrl} className="w-full aspect-video object-cover" />
              <button 
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase">{selectedItem.condition}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-4">
                 <div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">배송 방법</p>
                 ...
                 {selectedItem.deliveryMethod === DeliveryMethod.DELIVERY && (
                   <div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
                      <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">출발지 주소</p>
                      <p className="text-xs font-semibold text-gray-700 line-clamp-2">{selectedItem.originAddress}</p>
                   </div>
                 )}
              </div>
              ...
              <div className="bg-teal-50 rounded-2xl p-4 mb-6 border border-teal-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-teal-900">기부 약속: 수익금의 {selectedItem.donationPercent}%</p>
                    <p className="text-xs text-teal-700">구매 시 ₩ {(selectedItem.price * selectedItem.donationPercent / 100).toLocaleString()}이 우리 지역 아동 센터에 전달됩니다.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <button 
                  ...
                  className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-xl shadow-teal-100 hover:bg-teal-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  disabled={currentUser.points < selectedItem.price}
                >
      ...
      {/* Mobile Nav */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 md:hidden flex justify-around p-3 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button 
          onClick={() => setActiveTab('market')}
          className={`flex flex-col items-center gap-1 ${activeTab === 'market' ? 'text-teal-600' : 'text-gray-400'}`}
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
          <span className="text-[10px] font-bold">장터</span>
        </button>
        <button 
          onClick={() => setIsSelling(true)}
          className="flex flex-col items-center gap-1 -mt-8"
        >
          <div className="w-14 h-14 bg-gray-900 text-white rounded-full flex items-center justify-center shadow-xl border-4 border-white">
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
          </div>
        </button>
        <button 
          onClick={() => setActiveTab('stats')}
          className={`flex flex-col items-center gap-1 ${activeTab === 'stats' ? 'text-teal-600' : 'text-gray-400'}`}
        >
