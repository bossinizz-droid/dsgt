...
export default function SalesApp() {
  ...
  const [showColPicker, setShowColPicker] = useState(false)  // 컬럼 선택 드로어
  ...
  /* ─── 컬럼 카테고리 분류 ─── */
  const categorizeCol = (col, rows) => {
    const uniq = getUniqCount(rows, col)
    if (['년월','연월','회계연월','회계일','기표일','기표번호','전표번호','사업자번호'].some(k => col.includes(k))) return 'date'
    if (['금액','차변','대변','잔액'].some(k => col.includes(k))) return 'amount'
    if (uniq <= 30) return 'good'   // 집계하기 좋음
    if (uniq <= 100) return 'ok'
    return 'many'
  }
  ...
  return (
    <div className={styles.app}>
      ...
      <header className={styles.hdr}>
        ...
        {parsedData ? (
          <button className={styles.groupBtn} onClick={() => setShowColPicker(true)} title="집계 기준 변경">
      ...
      {parsedData && (<>
        ...
        <div className={styles.groupInfo}>
          <span className={styles.groupInfoLbl}>집계 기준</span>
          <span className={styles.groupInfoVal}>{groupCol}</span>
          <span className={styles.groupInfoCount}>{sortedTypes.length}개 그룹</span>
          <button className={styles.groupChangeBtn} onClick={()=>setShowColPicker(true)}>변경</button>
        </div>
      ...
      {/* ════════════════════════════════
          컬럼 선택 드로어
      ════════════════════════════════ */}
      <div className={`${styles.overlay} ${showColPicker?styles.overlayOn:''}`} onClick={()=>setShowColPicker(false)}/>
      <div className={`${styles.drawer} ${showColPicker?styles.drawerOpen:''}`}>
        <div className={styles.dHandle}/>
        <div className={styles.dHdr}>
          <span className={styles.dTitle}>집계 기준 컬럼 선택</span>
          <button className={styles.dClose} onClick={()=>setShowColPicker(false)}>✕</button>
        </div>
        <div className={styles.dBody}>
          <p className={styles.colPickerDesc}>
            선택한 컬럼의 고유값을 기준으로 매출·매입을 자동 집계합니다.<br/>
            고유값이 적을수록 보기 좋은 결과가 나옵니다.
          </p>

          {rawRows && (() => {
            const good=[], ok=[], many=[], skip=[]
            columns.forEach(col => {
              const cat = categorizeCol(col, rawRows)
              if (cat==='date'||cat==='amount') skip.push(col)
              else if (cat==='good') good.push(col)
              else if (cat==='ok') ok.push(col)
              else many.push(col)
            })
            return (
              <>
                {good.length>0 && <>
                  <div className={styles.colGroup}>
                    <span className={styles.colGroupLbl}>추천 — 집계하기 좋은 컬럼</span>
                  </div>
                  {good.map(col => <ColItem key={col} col={col} rows={rawRows} active={groupCol===col} onClick={()=>changeGroupCol(col)} recommend/>)}
                </>}
                {ok.length>0 && <>
                  <div className={styles.colGroup}><span className={styles.colGroupLbl}>사용 가능</span></div>
                  {ok.map(col => <ColItem key={col} col={col} rows={rawRows} active={groupCol===col} onClick={()=>changeGroupCol(col)}/>)}
                </>}
                {many.length>0 && <>
                  <div className={styles.colGroup}><span className={styles.colGroupLbl}>고유값 많음 (그룹이 많아질 수 있음)</span></div>
                  {many.map(col => <ColItem key={col} col={col} rows={rawRows} active={groupCol===col} onClick={()=>changeGroupCol(col)} warn/>)}
                </>}
              </>
            )
          })()}
      ...
      {/* ════════════════════════════════
          상세 드로어
      ════════════════════════════════ */}
      <div className={`${styles.overlay} ${drawer&&!showColPicker?styles.overlayOn:''}`} onClick={()=>setDrawer(null)}/>
      <div className={`${styles.drawer} ${drawer&&!showColPicker?styles.drawerOpen:''}`}>
        <div className={styles.dHandle}/>
        <div className={styles.dHdr}>
          <div>
...
/* ─── 컬럼 선택 아이템 컴포넌트 ─── */
function ColItem({ col, rows, active, onClick, recommend, warn }) {
  const uniq = getUniqCount(rows, col)
  const samples = [...new Set(rows.slice(0,200).map(r=>String(r[col]||'').trim()).filter(Boolean))].slice(0,4)
  return (
    <div className={`${styles.colItem} ${active?styles.colItemActive:''}`} onClick={onClick}>
      <div className={styles.colItemLeft}>
        <div className={styles.colItemName}>
          {col}
          {recommend && <span className={styles.colTag} style={{background:'#e6f4ec',color:'#145a32'}}>추천</span>}
          {warn && <span className={styles.colTag} style={{background:'#fef3c7',color:'#78350f'}}>많음</span>}
        </div>
        <div className={styles.colItemSample}>{samples.join(' · ')}{samples.length<uniq?` 외 ${uniq-samples.length}개`:''}</div>
      </div>
      <div className={styles.colItemRight}>
        <div className={styles.colItemUniq}>{uniq}개</div>
        {active && (
          <svg viewBox="0 0 24 24" fill="none" stroke="#1a6ef5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{width:18,height:18}}>
